...
class SECFilingsRequest(pydantic.BaseModel):
    ...
    year: Annotated[
        int,
        "The year for getting the documents",
        pydantic.Field(alias="year", ge=2000, le=datetime.datetime.now().year),
    ]
...
class AccessionNumElem(pydantic.BaseModel):
    ...
    @classmethod
    def from_accession_metadata(
        cls, accession_num: str, filing_name: str, filing_date: str, report_date: str
    ) -> "AccessionNumElem":
...
class HTMLURLList(pydantic.BaseModel):
    ...
    @classmethod
    def from_cik_accnum_ticker(
        cls, cik: str, acc_num: "AccessionNumElem", ticker: str
    ) -> "HTMLURLList":
